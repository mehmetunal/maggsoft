...
using Maggsoft.Data.Mssql.Messages;
...
namespace Maggsoft.Mssql.Services.Messages;
...
public class SmtpBuilder(EmailAccountSettings emailAccountSettings, IEmailAccountService emailAccountService) : ISmtpBuilder
{
    ...
    private readonly EmailAccountSettings _emailAccountSettings = emailAccountSettings;
    private readonly IEmailAccountService _emailAccountService = emailAccountService;
    ...
    public virtual async Task<SmtpClient> BuildAsync(EmailAccount emailAccount = null)
    {
        if (emailAccount is null)
        {
            ...
            ?? throw new MaggsoftException("Email account could not be loaded");
        }
        ...
        {
            client.Dispose();
            throw new MaggsoftException(ex.Message, ex);
        }
    }
